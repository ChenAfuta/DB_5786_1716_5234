...
# שלב ד' – תכנות בבסיס הנתונים

בשלב זה הרחבנו את בסיס הנתונים באמצעות יכולות תכנות מתקדמות של PostgreSQL ו־PL/pgSQL.

בשלב נכתבו:

* 2 פונקציות
* 2 פרוצדורות
* 2 טריגרים (כולל טריגר UPDATE)
* 2 תוכניות ראשיות
* שימוש ב־Cursor מפורש ומשתמע
* שימוש ב־RECORD
* תנאי IF
* לולאות
* טיפול בחריגות באמצעות EXCEPTION
* פקודות DML מסוג INSERT / UPDATE / DELETE

---

# קבצי שלב ד'

קבצי שלב ד' נמצאים בתיקייה:
...
![Stage4 Files](stage_4/images/image6.png)

---

# קובץ AlterTable.sql

בוצעו הרחבות לטבלאות הקיימות לצורך תמיכה בתהליכי עיבוד מתקדמים, כולל:

* הוספת שדות סטטוס
* הוספת תאריכי עיבוד
* הוספת בדיקות תקינות (Constraints)

### הוכחת הרצה

הקובץ רץ בהצלחה ונוספו השדות החדשים למסד הנתונים.

![Alter Table](stage_4/images/image7.png)

---

# פונקציה 1 – Calculate Invoice Balance

## תיאור

פונקציה זו מחשבת את יתרת החשבונית לאחר חישוב תשלומים והחזרים.
...
```sql
fn_calculate_invoice_balance()
```

## הוכחת הרצה

בוצעה קריאה לפונקציה:

```sql
SELECT fn_calculate_invoice_balance(1);
```

הפונקציה רצה בהצלחה והחזירה יתרה תקינה.

![Function 1](stage_4/images/image1.png)

---

# פונקציה 2 – Validate Payment Amount

## תיאור

פונקציה זו בודקת האם סכום תשלום מסוים תקין ביחס לחשבונית.
...
```sql
fn_validate_payment_amount()
```

## הוכחת הרצה

בוצעה בדיקת תקינות עבור תשלום.

![Function 2](stage_4/images/image2.png)

---

# קובץ Procedures.sql

נוצרו שתי פרוצדורות לטיפול בתהליכים עסקיים בבסיס הנתונים.

### הוכחת יצירת הפרוצדורות

![Procedures Creation](stage_4/images/image8.png)

---

# פרוצדורה 1 – Process Refund

## תיאור

פרוצדורה זו מבצעת עיבוד החזר כספי.
...
נבדקה טבלת ההחזרים לאחר ביצוע הפעולה.

![Refund Results](stage_4/images/image4.png)

---

# פרוצדורה 2 – Update Insurance Claim Status

## תיאור

פרוצדורה זו מעדכנת סטטוס של תביעות ביטוח.
...
```sql
sp_update_insurance_claim_status()
```

## הוכחת הרצה

בוצעה בדיקה לאחר עדכון תביעות הביטוח.

![Insurance Claim Results](stage_4/images/image5.png)
...
1. טריגר Audit עבור תשלומים חדשים
2. טריגר UPDATE עבור שינוי סטטוס חשבונית

### הוכחת יצירת הטריגרים

![Trigger Creation](stage_4/images/image9.png)

---

# טריגר 1 – Payment Audit Trigger

## תיאור

הטריגר מופעל בעת הכנסת תשלום חדש ומעדכן טבלת Audit.
...
![Payment Insert](stage_4/images/image3.png)

---

# טריגר 2 – Update Invoice Trigger

## תיאור

הטריגר מופעל בעת UPDATE על חשבונית ומעדכן סטטוסים קשורים.
...
![Invoice Update Trigger](stage_4/images/image11.png)

---

# תוכניות ראשיות

נכתבו שתי תוכניות ראשיות:
...
בוצעה הרצה מלאה ללא שגיאות.

![Main Programs](stage_4/images/image10.png)

---

# סיכום

בשלב זה הורחב בסיס הנתונים כך שלא יכלול רק טבלאות וקשרים אלא גם לוגיקה עסקית מלאה.

באמצעות פונקציות, פרוצדורות, טריגרים ותוכניות ראשיות ניתן לבצע:
...
* טיפול בחריגות
* ביצוע תהליכים מורכבים ישירות מתוך בסיס הנתונים
